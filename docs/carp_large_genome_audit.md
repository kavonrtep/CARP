...
| B4 | `merge_repeat_annotations.R:34-40, :52`; `clean_rm_output.R:24` | `mclapply(as.list(gff_disjoin$revmap), …)` — one small vector per disjoint range + COW-forked full `gff`. **Comment records 296 GB max_rss at 3.9 Gb** → ~23× worse at 90 Gbp | Avoid `as.list`; vectorized `extractList`/`relist`+`unique`; dedupe revmap-group signatures so `resolve_name` runs once per class-combo; hard-cap workers. |
| B5 | `clean_rm_output.R:70`; `merge_repeat_annotations.R:94` | `read.table`/`import` of the **raw RM `.out`** (hundreds of millions of rows) into one data.frame, single-threaded | `data.table::fread` with `colClasses`, keep only needed columns. |
...
| B7 | `make_unified_annotation.R:1035` | `readLines(output_path)` on the **whole multi-GB unified GFF3** to insert 3 header lines | Write the provenance header before `export`, or prepend via streaming `cat`. |
...
| Q2 | `make_unified_annotation.R:630-642` | `resolve_tier1_overlaps`: `kept <- c(kept, piece)` grows a GRanges per iteration **and** `reduce()`s the whole growing set; one overlapping pair triggers a full-batch quadratic | Vectorize like `resolve_within_tier` (single `disjoin(with.revmap)`), or restrict to `queryHits/subjectHits`. |
| Q3 | `make_unified_annotation.R:130` | `subset_seqs` coerces the full seqnames Rle to character **per batch** (~450 batches × all tiers) | `split()` each tier by seqname once, `unlist()` per batch. |
