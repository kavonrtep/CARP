...
- Main computational environment: `tidecluster.yaml` (**DANTE_LTR 0.5.1.1**, RepeatMasker, and the R/report stack — used by ~19 rules). Despite the file name it no longer ships TideCluster, and **no longer ships DANTE**: DANTE moved to r-base 4.2.3 in 0.2.11 while dante_ltr 0.5.1.1 still pins r-dplyr 1.0.7 (r-base <4.2), so the two can no longer share an env. This env is pinned to r-base 4.1 by dante_ltr. (0.5.1.1 improves DANTE_LTR's efficiency on very large genomes; same deps as 0.5.1.0.)
- DANTE environment: `dante.yaml` (**DANTE 0.2.11**, r-base 4.2.3) — used only by the `dante` rule (the bare protein-domain search). Split out from `tidecluster.yaml` for the r-base conflict above.
...
- Specialized environment: `dante_tir.yaml` (**DANTE_TIR 0.2.7** tool for TIR transposon detection)
