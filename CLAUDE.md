...
- Main computational environment: `tidecluster.yaml` (**DANTE_LTR 0.5.2.0**, RepeatMasker, and the R/report stack — used by ~19 rules). Despite the file name it no longer ships TideCluster, and **no longer ships DANTE**: DANTE moved to r-base 4.2.3 in 0.2.11 while dante_ltr still pins r-dplyr 1.0.7 (r-base <4.2), so the two can no longer share an env. This env is pinned to r-base 4.1 by dante_ltr. (0.5.2.0 makes DANTE_LTR's library clustering **deterministic** — canonically sorts `TE_all.fasta` before `mmseqs easy-cluster`, so the Ty1_copia/Angela library no longer churns run-to-run; see `docs/archive/dante_ltr_deterministic_clustering_request.md`.)
