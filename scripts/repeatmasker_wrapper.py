...
def setup_rmblast_culling_shim(culling_limit, workdir):
    """Make RepeatMasker pass ``-culling_limit N`` to rmblastn without patching RM.

    RepeatMasker resolves both ``rmblastn`` and ``makeblastdb`` from its
    ``RMBLAST_DIR`` (overridable via the ``RMBLAST_DIR`` environment variable;
    see RepeatMaskerConfig.pm). There is no CLI flag to add extra rmblastn
    parameters, so we build a tiny shim directory holding a wrapper ``rmblastn``
    that appends ``-culling_limit N`` to real search invocations (those carrying
    ``-query``; ``-version`` and friends pass through untouched) plus a symlink
    to the real ``makeblastdb``, and point ``RMBLAST_DIR`` at it. The env var is
    inherited by every per-chunk RepeatMasker process spawned below.

    ``-culling_limit N`` discards an HSP whose query range is enveloped by >= N
    higher-scoring HSPs. On a redundant de-novo library (many near-identical
    consensi per family, so each genomic copy otherwise produces hundreds of
    overlapping HSPs) this collapses the per-locus HSP explosion and speeds
    RepeatMasker ~3x at a <1% masked-bp cost, with classification preserved
    (the best hit per locus is never culled).

    Returns the shim dir, or None when culling is off / rmblastn cannot be
    located — in which case the run proceeds unchanged (the optimisation is never
    a correctness requirement).
    """
    if not culling_limit or culling_limit <= 0:
        return None
    real_rmblastn = shutil.which('rmblastn')
    if real_rmblastn is None:
        print("WARNING: --culling-limit requested but rmblastn not found on "
              "PATH; running WITHOUT culling.")
        return None
    ...
    real_makeblastdb = shutil.which('makeblastdb') or os.path.join(real_dir,
                                                                   'makeblastdb')
    shim_dir = os.path.join(os.path.abspath(workdir), 'rmblast_culling_shim')
    ...
    with open(shim_rmblastn, 'w') as fh:
        fh.write(
            "#!/bin/bash\n"
            "# Auto-generated by repeatmasker_wrapper.py; appends -culling_limit\n"
            ...
            '  fi\n'
            'done\n'
            'exec "$REAL" "$@"\n' % (real_rmblastn, int(culling_limit))
        )
